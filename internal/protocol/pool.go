...
func (p *Pool) GetHostSign(ctx context.Context, loginBuffer, appID string, payload map[string]any, accountID int64, tcpProxy string) (map[string]any, error) {
	return p.run(ctx, loginBuffer, accountID, tcpProxy, func(ctx context.Context, st WmpfSession) (map[string]any, error) {
		plain, err := buildHostSignRequest(st.Session.UIN, appID, payload)
		if err != nil {
			return nil, err
		}
		envelope, err := buildTransferPacket(st.Session, plain)
		...
		return parseHostSignResponse(code, resp)
	})
}
